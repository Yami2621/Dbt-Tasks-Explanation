{{
    config(
        alias= env_var('reconciliation_m1_report')
)}}
...
QUALIFY row_number() over(partition by id order by metadata.kafka_metadata.offset desc) = 1 
),
...
where date(_partitiontime) is not null
row_number() over(partition by id order by metadata.kafka_metadata.offset desc) = 1
),
...
where date(_partitiontime) is not null
QUALIFY row_number() over(partition by consultant_id,region_id  order by metadata.kafka_metadata.offset desc) = 1
),
...
where date(_partitiontime) is not null
QUALIFY row_number() over(partition by uuid order by metadata.kafka_metadata.offset desc) = 1 
),
